...
% upper-y - boundary condition	
	CEx(ysteps,1:xsteps)= (Ez(1,1:xsteps)-Ez(ysteps,1:xsteps))/dely;

	CEy(1:ysteps, 1:xsteps-1) = diff(Ez,1,2)/delx;
% right-x boundary condition
	CEy(1:ysteps, xsteps) = (Ez(1:ysteps,1) - Ez(1:ysteps,xsteps))/delx;
%update Hx	
	Hx+=up_hx.*CEx;
%update Hy
	Hy+=up_hy.*CEy;
...
%boundary at low y
	CHz(1,2:xsteps)= diff(Hy(1,:))/delx - (Hx(1,2:xsteps)-Hx(ysteps,2:xsteps))/dely;
%boundaty at low x
	CHz(2:ysteps,1)= (Hy(2:ysteps,1)-Hy(2:ysteps,xsteps))/delx - diff(Hx(:,1))/dely;

%corner boundaty at (1, 1) 
	CHz(1, 1) = (Hy(1,1)-Hy(ysteps,1))/delx - (Hx(1,1)-Hx(1,xsteps))/dely;
